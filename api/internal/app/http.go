...
func parseBody[T any](w http.ResponseWriter, r *http.Request, t *T) error {
	if err := json.NewDecoder(r.Body).Decode(t); err != nil {
		logger.GetLogger(r.Context()).Errorf("failed to decode request body: %v", err)
	...
	if err := validate.Struct(t); err != nil {
		logger.GetLogger(r.Context()).Errorf("validation error: %v", err)
		e := newErrorResponse(err.Error())
...
func parseURLParamUint(w http.ResponseWriter, r *http.Request, param string) (uint, error) {
	...
	if paramValue == "" {
		logger.GetLogger(r.Context()).Errorf("missing URL parameter: %s", param)
	...
	if err != nil {
		logger.GetLogger(r.Context()).Errorf("invalid URL parameter %s: %v", param, err)
...
func getAccessToken(w http.ResponseWriter, r *http.Request) (*auth.AccessToken, error) {
	...
	if err != nil {
		logger.GetLogger(r.Context()).Errorf("failed to get access token: %v", err)
